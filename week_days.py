...
def favorite_activities():
    ...
    for day in days:
            ...
            favorite_things = input(f"What is your favorite activitiy to do on {day}s? ")
            favorite_activities.append(favorite_things)
    ...
    # 2.2 TODO: ABOVE your for loop, create a new empty list to hold the user's favorite activities.x

    # 2.3 TODO: Now, back in your for loop, append each of the user's answers into your new list.x
    # AFTER your loop, print out the list to check if it got populated correctly.
    for day in range(len(days)):
        print(f'On {days[day]}s, your favorite activity is to {favorite_activities[day]}.')
...
def temp_by_day():
    ...
    for day in range(len(days)):
            climate = int(input(f'What is the temp on {days[day]}'))
            
            if climate < 50:
                 print('Brr, put on a jacket!')
            elif climate >= 50 and climate <= 65:
...
def temp_by_day_continuous():
    ...
    # Spring has sprung!
    temp_outside = int(input("what temperature is it outside?"))
    while (temp_outside < 65):
         print("Wear a sweater")
         temp_outside = int(input("what temperature is it outside?"))
    print("Spring has sprung!")
    # Edddie and Asha helped debug the code
